...
HOR:: Get_Bass_Line()
{
    ...
    if ((Line_Bass_Note[pos] == 0) && (basspending == 0))
        return;
    ...
    if (pos != lpos) {
        ...
        if (Line_Bass_Note[pos] != 0) {
            ...
            if (lanota == 5)
                lanota += TCh[chord_type].third;
            if (lanota == 8)
                lanota += TCh[chord_type].five_th;
            if (lanota == 12)
                lanota += TCh[chord_type].seven_th;
            ...
            if (bass_note < 0)
                bass_note += 12;
            length_bass_note =
                (((AB[bass_note].tune + AB[bass_note].bmt) * 44100.0) / SAMPLE_RATE);
...
int HOR::Select_Rhythm(char *nomrit)
{
...
void HOR::Select_Bass(char *nombass)
{
...
void HOR::Get_Bass()
{
    ...
    if ((readcounts + longi) < frames_bass)
        basspending = 1;
