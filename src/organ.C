...
int i, j, l1, k, Pexitprogram, MidiInLevel, LastMidiInLevel, UndoCount, preset,
    Signal_for_Cb_Sliders, BarLead, changeNameChord;
char NameChord[16];
...
HOR::HOR()
{
    ...
    int tcombl[16] = {10586, 12340, 6400, 13100, 8004, 7200, 5130,  9037,
                      12045, 11237, 9456, 7634,  5389, 8056, 10120, 11432};
    int tcombr[16] = {10518, 11340, 8450, 11100, 9644, 7560, 9536, 11507,
                      12600, 11111, 8056, 6048,  7690, 5978, 8845, 10056};
    int tapsg[16] = {36, 33, 29, 27, 24, 21, 17, 15,
                     13, 16, 21, 24, 27, 31, 33, 36};
    ...
    for (i = 0; i < 16; i++)
        ready_apsg[i] = (float)apsg[i] / (float)apss;
...

    {
        ...
        if (i > 0)
            lsin[i - 1] = (lsin[i - 1] * (1.0 + lsin[i] - lsin[i - 1]));
        if (i > 1)
            lsin[i - 2] = (lsin[i - 2] * (1.0 + lsin[i - 1] - lsin[i - 2]));
        if (i > 2)
            lsin[i - 3] = (lsin[i - 3] * (1.0 + lsin[i - 2] - lsin[i - 3]));
        if (i > 3)
            lsin[i - 4] = (lsin[i - 4] * (1.0 + lsin[i - 3] - lsin[i - 4]));
        if (i > 4)
            lsin[i - 5] = (lsin[i - 5] * (1.0 + lsin[i - 4] - lsin[i - 5]));
        if (i > 5)
            lsin[i - 6] = (lsin[i - 6] * (1.0 + lsin[i - 5] - lsin[i - 6]));
        if (i > 6)
            lsin[i - 7] = (lsin[i - 7] * (1.0 + lsin[i - 6] - lsin[i - 7]));
        if (i > 7)
            lsin[i - 8] = (lsin[i - 8] * (1.0 + lsin[i - 7] - lsin[i - 8]));
    }
    ...
    for (i = 0; i < (int)sizesin; i++) {
        ...
        if (i > 0)
            msin[i - 1] = (msin[i - 1] * (1.0 + msin[i] - msin[i - 1]));
        if (i > 1)
            msin[i - 2] = (msin[i - 2] * (1.0 + msin[i - 1] - msin[i - 2]));
        if (i > 2)
            msin[i - 3] = (msin[i - 3] * (1.0 + msin[i - 2] - msin[i - 3]));
        if (i > 3)
            msin[i - 4] = (msin[i - 4] * (1.0 + msin[i - 3] - msin[i - 4]));
        if (i > 4)
            msin[i - 5] = (msin[i - 5] * (1.0 + msin[i - 4] - msin[i - 5]));
        if (i > 5)
            msin[i - 6] = (msin[i - 6] * (1.0 + msin[i - 5] - msin[i - 6]));
        if (i > 6)
            msin[i - 7] = (msin[i - 7] * (1.0 + msin[i - 6] - msin[i - 7]));
        if (i > 7)
            msin[i - 8] = (msin[i - 8] * (1.0 + msin[i - 7] - msin[i - 8]));
        ...
        ssin[i] =
            (lsin[i] + nsin[i] + msin[i] + psin[i] + qsin[i] + rsin[i] + tsin[i]) / 7.0;
        if (i > 0)
            ssin[i - 1] = (ssin[i - 1] * (1.0 + ssin[i] - ssin[i - 1]));
        if (i > 1)
            ssin[i - 2] = (ssin[i - 2] * (1.0 + ssin[i - 1] - ssin[i - 2]));
        if (i > 2)
            ssin[i - 3] = (ssin[i - 3] * (1.0 + ssin[i - 2] - ssin[i - 3]));
        if (i > 3)
            ssin[i - 4] = (ssin[i - 4] * (1.0 + ssin[i - 3] - ssin[i - 4]));
        if (i > 4)
            ssin[i - 5] = (ssin[i - 5] * (1.0 + ssin[i - 4] - ssin[i - 5]));
        if (i > 5)
            ssin[i - 6] = (ssin[i - 6] * (1.0 + ssin[i - 5] - ssin[i - 6]));
        if (i > 6)
            ssin[i - 7] = (ssin[i - 7] * (1.0 + ssin[i - 6] - ssin[i - 7]));
        if (i > 7)
            ssin[i - 8] = (ssin[i - 8] * (1.0 + ssin[i - 7] - ssin[i - 8]));
...
float HOR::Get_Keyb_Level_Scaling(int nota)
{
...
float HOR::pitch_Operator(int i, int note)
{
    return (lasfreq[a[0].Operator[i].harmonic] + a[0].Operator[i].harmonic_fine);
}


float HOR::pitch_Operator2(int i, int note)
{
    return (lasfreq[a[0].Operator[i].harmonic] - a[0].Operator[i].harmonic_fine);
}
...
void HOR::volume_Operator(int i, int l2)
{
    a[0].Operator[i].con1 = a[0].Operator[i].volumen * velocity[l2] *
                            a[0].Normalize[a[0].Operator[i].harmonic];
};
...
float HOR::Cenvelope(int *note_active, int gate, float t, int nota)
{

    float val = 0;

    if (t > a[0].c_attack + a[0].c_decay)
        return 0.0f;
    if (t > a[0].c_attack)
        val = 1.0 - (t - a[0].c_attack) * u_c_decay;
    else
        val = t * u_c_attack;

    if (gate)
        return (a[0].Click_Vol * val);
    return (a[0].Click2_Vol * val);
};


float HOR::Penvelope(int *note_active, int gate, float t, int nota)
{

    if ((gate) || (pedal)) {
        if (t > a[0].p_attack + a[0].p_decay)
            return 0.0f;
        if (t > a[0].p_attack)
            return (1.0 - (t - a[0].p_attack) * u_p_decay);
        return (t * u_p_attack);
    }
    else
...
float HOR::Jenvelope(int *note_active, int gate, float t, int nota)
{

    float Env = 0.0f;

    if (gate) {
        if (t > a[0].attack + a[0].decay)
            return (a[0].sustain);
        if (t > a[0].attack)
            return (1.0 - (1.0 - a[0].sustain) * (t - a[0].attack) * u_decay);
        ...
        if (pedal == 0) {
            ...
            if (a[0].sustain != 0)
                ...
                return (1.0 - (t - a[0].attack) * u_decay);
        }
    }
...
float HOR::Get_Partial(int nota)
{
    int l;
    float partial = 0;
    float freq_note = 0;
    ...
    freq_note = (pitch > 0) ? h[l].f2 + (h[l].f3 - h[l].f2) * pitch :
                              h[l].f2 + (h[l].f2 - h[l].f1) * pitch;
...
void HOR::Alg1s(int nframes, void *)
{
    ...
    for (l2 = 0; l2 < POLY; l2++) {
        ...
        if (note_active[l2]) {
            ...
            for (l1 = 0; l1 < PERIOD; l1 += 2) {
                ...
                Perc_Volume[l2] =
                    Penvelope(&note_active[l2], gate[l2], env_time[l2], l2);
